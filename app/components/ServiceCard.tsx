...
const ServiceCard: React.FC<ServiceCardProps> = ({ imageSrc, serviceName }) => {
  return (
    <div className="relative h-[461px] w-full sm:w-[410px] overflow-hidden group cursor-pointer">
      ...
      {/* Hover Overlay */}
      <div className="absolute inset-0 bg-[#495D4D] opacity-0 group-hover:opacity-95 transition-opacity duration-300 flex items-center justify-center">
        <h3 className="text-[#F0E8C6] text-3xl font-logga font-extrabold tracking-wider drop-shadow-lg">{serviceName}</h3>
      </div>
    </div>
  );
};
