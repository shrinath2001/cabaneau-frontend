...
const CabinCard: React.FC<CabinCardProps> = ({
  ...
  return (
    <div className="bg-white shadow-md overflow-hidden w-full sm:w-96 shrink-0 border border-black  p-4">
      <div className="relative h-48">
        <Image src={imageSrc} alt={title} layout="fill" objectFit="cover" />
        {/* Image navigation arrows - Placeholder for now */}
        <div className="absolute inset-0 flex items-center justify-between p-2">
          <button className=" text-white p-1 rounded-full">{'<'}</button>
          <button className=" text-white p-1 rounded-full">{'>'}</button>
        </div>
      </div>
      <div className="py-4">
        ...
        <div className="text-sm font-heading font-normal text-black mb-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex space-x-2 text-black-500">
              <span>🏠</span> {/* Placeholder for area icon */}
              <span>👥</span> {/* Placeholder for capacity icon */}
              <span>🚿</span> {/* Placeholder for another amenity */}
            </div>
          </div>
        </div>
        ...
        <button className="w-full py-2 px-4 border  border-green-800 hover:border-white text-green-800 hover:bg-orange-400 hover:text-white transition-colors">
          BOOK NOW
        </button>
      </div>
    </div>
