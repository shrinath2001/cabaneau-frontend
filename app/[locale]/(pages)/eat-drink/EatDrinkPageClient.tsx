...
interface EatDrinkPageClientProps {
...
/**
 * Owns the tab/modal/sticky-scroll interactivity. dining/breakfast/drinks/
 * pageData arrive already fetched server-side (see page.tsx), so every
 * item's text is present in the server HTML regardless of which tab is
 * visually active - the tab state below only filters what's already
 * rendered, it doesn't gate the fetch.
 */
export default function EatDrinkPageClient({ dining, breakfast, drinks, pageData }: EatDrinkPageClientProps) {
    ...
    <main>
      ...
      <section ref={discoverSectionRef} className="grid grid-cols-1 md:grid-cols-2">
        ...
        {(() => {
          ...
          return (
            <div className="relative h-[300px] md:h-[400px] flex flex-col items-center justify-center">
              <div className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: `url(${slot1Image})` }}>
                <div className="absolute inset-0" style={{ backgroundColor: 'rgba(0, 0, 0, 0.50)' }}></div>
              </div>
              {slot1Text && (
        ...
        {(() => {
          ...
          return (
            <div className="relative h-[300px] md:h-[400px] flex flex-col items-center justify-center">
              <div className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: `url(${slot2Image})` }}>
                <div className="absolute inset-0" style={{ backgroundColor: 'rgba(0, 0, 0, 0.50)' }}></div>
              </div>
              {slot2Text && (
                <h2 className="relative z-10 text-white text-4xl md:text-5xl lg:text-6xl font-custom text-center px-4 mb-6">
      ...
      {/* Detail Modal */}
      <EatDrinkDetailModal item={selectedItem} isOpen={isModalOpen} onClose={handleCloseModal} />
    </main>
  );
}
