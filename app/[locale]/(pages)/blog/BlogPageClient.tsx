...
/**
 * Owns the search/category-filter/pagination interactivity. The first
 * page of posts (default filters) arrives already fetched server-side
 * (see page.tsx) so a crawler sees real titles/excerpts on first paint;
 * the fetch effects below still run on mount to keep the existing
 * client-side filtering behaviour exactly as it was, just re-fetching
 * data that's already present rather than fetching it for the first time.
 */
export default function BlogPageClient({
  ...
  const { t, locale } = useTranslations('blog');
  const [posts, setPosts] = useState<BlogPost[]>(initialPosts);
  const [categories, setCategories] = useState<BlogCategory[]>(initialCategories);
  const [allPosts, setAllPosts] = useState<BlogPost[]>(initialAllPosts); // For category counts
  const [loading, setLoading] = useState(false);
  const [searchQuery, setSearchQuery] = useState('');
  const [debouncedSearch, setDebouncedSearch] = useState('');
  const [selectedCategory, setSelectedCategory] = useState<string | null>(null);
  const [currentPage, setCurrentPage] = useState(1);
  const [totalPages, setTotalPages] = useState(Math.ceil(initialTotal / POSTS_PER_PAGE));
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      ...
      try {
        ...
        setTotalPages(Math.ceil((result.total || 0) / POSTS_PER_PAGE));
      } catch (error) {
